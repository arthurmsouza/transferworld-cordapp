package net.corda.transferworld
...
fun main(args: Array<String>) {
    ...
    driver(isDebug = true) {
        startNode(
                providedName = CordaX500Name("Notary", "London", "GB"),
                advertisedServices = setOf(ServiceInfo(SimpleNotaryService.type)))
        val (nodeA, nodeB) = listOf(
                startNode(providedName = CordaX500Name("Arthur", "London", "GB"), rpcUsers = listOf(user)),
                startNode(providedName = CordaX500Name("Bueno", "New York", "US"), rpcUsers = listOf(user))).map { it.getOrThrow() }
